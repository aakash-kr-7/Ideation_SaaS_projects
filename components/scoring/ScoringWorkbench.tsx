...
export function ScoringWorkbench({reports}:{reports:CompletedScorecard[]}){
  ...
  if(!report||!weights||!scorecard)return <div className="scoring-workbench"><div className="workbench-intro"><div><p className="eyebrow">Interactive decision model · Principal</p><h2>No completed validations yet.</h2><p>Complete a validation before adjusting its score breakdown.</p><Link className="button button-small" href="/research/new">Validate an idea <ArrowRight size={14}/></Link></div></div></div>;
