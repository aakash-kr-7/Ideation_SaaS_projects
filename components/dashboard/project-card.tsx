...
export function ProjectCard({ run }: { run: ResearchRun }) {
  ...
  return <article className={`project-card group ${motion.cardInteractive}`}><div className="project-top"><span className="mini-mark">{run.ideaName.slice(0,2).toUpperCase()}</span>{o && <ScoreBadge score={o.score.total} size="sm"/>}</div><h3>{run.ideaName}</h3><p>{run.ideaDescription}</p><div className="project-meta"><span><Calendar size={13}/> {run.createdAt}</span>{o && <VerdictBadge verdict={o.verdict}/>}</div><Link href={href} className="project-card-link">{run.status === "Completed" ? "Open report" : "View progress"} <ArrowUpRight size={14}/></Link></article>;
}
