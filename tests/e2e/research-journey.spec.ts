...
test.describe("public and authenticated route contracts", () => {
  test("landing page exposes the two canonical report modes", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: /Stop building on assumptions/i })).toBeVisible();
  ...
  test("sample reports render both current schemas", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Experience the difference in evidence depth" })).toBeVisible();
