...
const reservationResultSchema = z.object({
  ...
  run_status: z.string(),
  report_mode: z.enum(["quick_scan", "full_validation"]),
  credit_cost: z.number().int(),
  credit_source: z.enum(["free_monthly", "paid"]),
  available_paid_credits: z.number().int().nonnegative(),
  free_quick_scans_remaining: z.number().int().min(0).max(1),
  duplicate: z.boolean(),
});

const creditSnapshotSchema = z.object({
  team_id: z.string().uuid(),
  paid_credits: z.number().int().nonnegative(),
  reserved_paid_credits: z.number().int().nonnegative(),
  free_quick_scans_remaining: z.number().int().min(0).max(1),
  quick_scans_available: z.number().int().nonnegative(),
  full_validations_available: z.number().int().nonnegative(),
  free_cycle_started_at: z.string(),
});
...
function databaseError(error: { message?: string } | null, requestId: string) {
  ...
  if (message.includes("INSUFFICIENT_CREDITS")) {
    return new ResearchLaunchError(
      "INSUFFICIENT_CREDITS",
      "You do not have enough credits for this report.",
      requestId,
      402,
    );
  }
  if (message.includes("PROJECT_ACCESS_DENIED")) {
    return new ResearchLaunchError(
      "PROJECT_ACCESS_DENIED",
      "This project is not available to your team.",
      requestId,
      403,
    );
  }
...
export const ResearchService = {
  async startResearchRun(input: z.infer<typeof startResearchRunSchema>) {
    ...
    const { data: reservationData, error: reservationError } = await (supabase as any)
      .rpc("create_research_run_with_reservation", {
        ...
        p_target_customer: validated.target_customer,
        p_market_type: validated.market_type,
        p_target_region: validated.target_region,
        p_assumptions: validated.assumptions,
        p_mode: validated.mode,
        p_idempotency_key: validated.idempotency_key,
        p_request_id: requestId,
      });
    if (reservationError) throw databaseError(reservationError, requestId);
    const reservation = reservationResultSchema.parse(firstRow(reservationData));
    ...
    if (!workerSecret || !supabaseUrl) {
      ...
      throw new ResearchLaunchError(
        ...
        "Research processing is temporarily unavailable. Your reserved credit was restored.",
        requestId,
        503,
      );
    }
    ...
    console.info(JSON.stringify({
      event: "research_run_dispatched",
      requestId,
      runId: reservation.run_id,
      userId: user.id,
      reportMode: validated.mode,
  ...
  async getResearchRuns(projectId: string) {
    return ResearchRepository.getProjectRuns(projectId);
  },
};
