...
function buildReport(mode: "quick_scan" | "full_validation"): ValidationReport {
  ...
  return validationReportSchema.parse({
    ...
    retrieval: { frozenFixture: true, sourcesAnalyzed: new Set(usedEvidence.map(item => item.url)).size, independentlyAddressableSources: new Set(usedEvidence.map(item => item.url)).size, researchPasses: full ? 3 : 2 },
