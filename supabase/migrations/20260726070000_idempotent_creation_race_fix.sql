-- Fix concurrent idempotency check race in create_research_run_with_reservation.
-- Advisory lock on (user_id, idempotency_key) ensures parallel requests for the same
-- idempotency key wait cleanly and return the existing run with duplicate=true instead
-- of violating unique constraints.
...
begin
  ...
  -- Acquire transaction lock on (user_id, idempotency_key) to serialize concurrent requests with identical idempotency keys.
  perform pg_advisory_xact_lock(hashtext('idempotency:' || v_user_id::text || ':' || p_idempotency_key::text));
...
revoke all on function public.create_research_run_with_reservation(uuid,text,text,text,text,text,jsonb,public.report_mode,uuid,uuid) from public, anon, authenticated;
grant execute on function public.create_research_run_with_reservation(uuid,text,text,text,text,text,jsonb,public.report_mode,uuid,uuid) to service_role, authenticated;
