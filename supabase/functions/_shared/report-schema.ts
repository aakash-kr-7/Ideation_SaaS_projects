...
export const evidenceSchema = z.object({ id: z.string(), source: z.string(), sourceType: z.string(), title: z.string(), snippet: z.string(), url: z.string().url().or(z.string().startsWith("/")), signal: z.enum(["Pain", "Demand", "Pricing", "Risk"]), strength: z.enum(["High", "Medium", "Low"]), date: z.string(), evidenceFamily: z.enum(["problem", "solution"]).optional(), researchPass: z.number().int().min(1).max(3).optional(), sourceTier: z.number().int().min(1).max(4).optional(), excluded: z.boolean().optional(), disconfirming: z.boolean().optional(), painPoint: z.string().optional(), independentSourceCount: z.number().int().nonnegative().optional(), independentDomainCount: z.number().int().nonnegative().optional() });
...
const marketSizeEntrySchema = z.object({ figure: z.string(), evidenceItemId: z.string(), sourceId: z.string(), citationUrl: z.string().url() }).nullable();
export const marketSizingSchema = z.object({ TAM: marketSizeEntrySchema, SAM: marketSizeEntrySchema, SOM: marketSizeEntrySchema, MarketSize: marketSizeEntrySchema, reason: z.string().nullable() });
export const validationReportSchema = z.object({ id: z.string(), version: z.literal("1.0"), generatedAt: z.string(), executiveSummary: z.string(), opportunity: opportunitySchema, methodology: z.string(), marketSizing: marketSizingSchema.optional(), retrieval: z.record(z.string(), z.unknown()).optional() });
...
export interface ValidationReport { id: string; version: "1.0"; generatedAt: string; executiveSummary: string; opportunity: ReportOpportunity; methodology: string; marketSizing?: z.infer<typeof marketSizingSchema>; retrieval?: Record<string, unknown>; }
