...
export function compareSpecialistAndChecker(
  ...
  const checkerDirection = checkerResult?.status === "Complete"
    ...
    : "Unavailable";
  if (specialistDirection === "Unavailable" || checkerDirection === "Unavailable") {
    return {
      specialist,
      specialistDirection,
      checkerDirection,
      disputed: true,
      reason: "The independent interpretation could not be completed, so this specialist conclusion is not independently reproduced.",
    };
  }
  if (specialistDirection !== checkerDirection) {
    return {
      ...
      checkerDirection,
      disputed: true,
      reason: `Disputed interpretation: specialist concluded ${specialistDirection}, while the isolated checker concluded ${checkerDirection}.`,
    };
  }
  return {
    specialist,
    ...
    checkerDirection,
    disputed: false,
    reason: `Independent checker reproduced the ${specialistDirection} direction.`,
  };
}
...
): { effectiveVerdict: EngineVerdict; adversarialDowngrade: boolean; reason: string | null } {
  const unresolved = adversarialGate.outcome === "StrongObjection" &&
    (adversarialGate.severity === "High" || adversarialGate.severity === "Medium");
  if (!unresolved || VERDICT_ORDER.indexOf(deterministicVerdict) <= VERDICT_ORDER.indexOf("Weak Signal")) {
    return { effectiveVerdict: deterministicVerdict, adversarialDowngrade: false, reason: null };
  }
  return {
    effectiveVerdict: "Weak Signal",
    adversarialDowngrade: true,
    reason: `The deterministic ${deterministicVerdict} tier is blocked by an unresolved evidence-cited adversarial objection.`,
  };
}
...
export function validateNarrativeCitations(
  ...
  const executiveSummary = validateSection("executive_summary", judge.executive_summary);
