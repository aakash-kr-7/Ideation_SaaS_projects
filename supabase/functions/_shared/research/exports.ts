    ...
    specialistOutputs?: Array<
      {
        ...
        confidence?: string;
        relevantBriefDimensions?: string[];
        unresolvedGaps?: string[];
...
function citationLabels(payload: unknown) {
  ...
  return new Map(evidence.map((item, index) => [
    String(item.id || ""),
    `[S${index + 1}] ${item.title || item.source || "Evidence source"}${
      item.source ? ` (${item.source})` : ""
    }`,
  ]));
}
function humanCitations(
...
function decisionProductMarkdown(payload: unknown) {
  ...
  const specialists = (product.specialistOutputs || []).map((item) =>
    `### ${item.name}\n\n- Confidence: ${item.confidence}\n- Findings: ${
      ...
      humanCitations(item.evidenceIds, labels)
    }\n- Relevant brief dimensions: ${
      (item.relevantBriefDimensions || []).join(", ") || "Not established"
    }\n- Opposing evidence: ${
      humanCitations(item.opposingEvidenceIds, labels, "None")
    }\n- Unresolved gaps: ${
      (item.unresolvedGaps || []).join("; ") || "None"
    }\n- Decision implication: ${item.decisionImplication}`
  ).join("\n\n");
  const charts = (product.charts || []).map((item) =>
    `- **${item.title || item.key}:** ${
      item.unavailable ? "Unavailable. " : ""
    }${item.sourceExplanation} Evidence: ${
      humanCitations(item.evidenceIds, labels, "none")
    }; data: ${JSON.stringify(item.sourceData || {})}`
  ).join("\n");
  return `# Decision dossier\n\n${
    product.headline || ""
  }\n\n**Evidence Confidence:** ${
    product.evidenceConfidence?.band || "Not recorded"
  }${
    typeof product.evidenceConfidence?.score === "number"
      ? ` (${product.evidenceConfidence.score})`
      : ""
  } — ${
    product.evidenceConfidence?.explanation || ""
...
- Recommended segment: ${
    verdict.recommendedTargetSegment ?? "Not supported yet"
  }
...
### Segment rankings
${
    (decision.segmentRankings || []).map((item: any, index: number) =>
      `${index + 1}. ${item.segment}: ${item.score} — ${item.rankReason}`
    ).join("\n") || "No supported segment ranking."
  }

### Scenario economics
${
    (decision.economicsScenarios || []).map((item: any) =>
      `- ${item.name}: price ${item.price ?? "unresolved"} ${
        item.currency ?? ""
      }; customers ${item.customersRequired ?? "unresolved"}; gross margin ${
        item.grossMarginRange?.join("–") ?? "unresolved"
      }; break-even ${item.breakEvenCustomers ?? "unresolved"}`
    ).join("\n")
  }

### 30-day validation plan
${
    (decision.founderActionPlan?.days || []).map((item: any) =>
      `- Days ${item.days}: ${item.action}`
    ).join("\n")
  }`;
}
...
export function renderMarkdown(input: ExportBundleInput) {
  ...
  const researchState =
    integrity.researchAvailabilityState === "insufficient_evidence"
      ? "Insufficient Evidence"
      : integrity.researchAvailabilityState === "research_completed"
      ? "Research Completed"
      : "Legacy report";
  let packs =
    integrity.researchExecution?.packStatuses?.map((item) =>
      `- ${(item.packKey || "").replaceAll("_", " ")}: ${
        (item.status || "").replaceAll("_", " ")
      } (${item.acceptedEvidenceCount ?? 0} accepted)`
    ).join("\n") || "Not persisted.";
  const funnel = integrity.researchExecution?.funnel;
  const funnelMarkdown = funnel
    ? `### Research funnel\n\n- Maximum sources reviewed: ${
      integrity.researchExecution?.reviewBudgetMaximum ?? "Not recorded"
    }\n- Sources discovered: ${
      funnel.sourcesDiscovered ?? 0
    }\n- Sources reviewed: ${funnel.sourcesReviewed ?? 0}\n- Sources fetched: ${
      funnel.sourcesFetched ?? 0
    }\n- Findings accepted: ${
      funnel.findingsAccepted ?? 0
    }\n- Findings rejected: ${
      funnel.findingsRejected ?? 0
    }\n- Independent evidence groups: ${
      funnel.independentEvidenceGroups ?? 0
    }\n- Direct or official sources: ${
      funnel.directOrOfficialSources ?? 0
    }\n- Challenging findings: ${funnel.challengingFindings ?? 0}\n\n`
    : "";
  packs = `${funnelMarkdown}${packs}`;
  const sourceRows = (integrity.opportunity?.evidence || []).map((item) =>
    `| ${item.title || "Untitled source"} | ${
      item.canonicalDomain || item.source || "Domain unavailable"
    } | ${item.sourceType || "Unknown"} | ${
      item.evidenceRole || "Not recorded"
    } | ${(item.associatedFactorIds || []).join(", ") || "None"} | ${
      item.url || "Unavailable"
    } |`
  ).join("\n");
  const rows = input.breakdowns.map((b) =>
    `| ${b.criterion} | ${b.rawScore ?? b.score} | ${
      b.evidenceCoefficient ?? "Legacy"
    } | ${b.effectiveScore ?? b.score} | ${
      b.evidenceState ?? "Legacy"
    } | ${b.weight} | ${humanCitations(b.evidenceIds, labels, "None")} | ${
      (b.confidenceDeductions || []).join("; ") || "None"
    } | ${(b.unresolvedGaps || []).join("; ") || "None"} |`
  ).join("\n");
  const sufficiencyMarkdown = sufficiency
    ? `## Evidence Sufficiency\n\n- Accepted evidence: ${sufficiency.acceptedEvidenceCount}\n- Independent evidence groups: ${sufficiency.independentEvidenceGroups}\n- Independent domains: ${sufficiency.independentDomains}\n- Source-family coverage: ${
      (sufficiency.sourceFamilyCoverage || []).join(", ") || "None"
    ...
    [
      ...
      ...rows.map((b) =>
        [
          ...
          input.ideaName,
          input.total,
          (input.payload as any)?.opportunity?.scorecard?.scoreBand?.display ||
          `${input.total}/100`,
          input.verdict,
          input.confidence,
          b.criterion,
